...
function createElement(i) {
  ...
  if (completeLevels.levelsBeat["x" + search[1]][i - 1]) {
    ...
    el.style.boxShadow = `inset 0 0 18px ${colors["x" + search[1]]}, 0 0 18px ${
      colors["x" + search[1]]
    }`;
  }
  let header = document.querySelector("header");
  header.style.borderBottom = `2px solid ${colors["x" + search[1]]}`;
  header.style.boxShadow = `0 0.5em 2em black,0 0.5em 2em black`;
